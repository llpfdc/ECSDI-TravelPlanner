# -*- coding: utf-8 -*-
"""
filename: SimpleDirectoryAgentRL
...
def register():
    ...
    def process_register(content):
        """ Registro en el directorio
        """

        logger.info('Peticion de registro')

        # Extraemos el nombre del agente (FOAF.name), el URI del agente
    ...
    # REGLA 3: Si "La performativa es ACL.request" ENTONCES "extraemos la accion del mensaje"
    accion = gm.value(subject=msgdic['content'], predicate=RDF.type)

    # REGLA 4: SI "es una accion DSO.Register" ENTONCES "registramos y respondemos siguiendo el protocolo"
    if accion == DSO.Register:
        return process_register(msgdic['content']).serialize(format='xml')


    # REGLA 5: SI "es una accion DSO.Search" entonces "buscamos y respondemos siguiendo el protocolo"
    if accion == DSO.Search:
        return process_search(msgdic['content']).serialize(format='xml')
    
    # REGLA 6: SI "no habia ninguna accion en el mensaje" ENTONCES "Mensaje no entendido"
    return build_message(Graph(),
                        ACL['not-understood'],
                        sender=DirectoryAgent.uri,
                        msgcnt=mss_cnt).serialize(format='xml')
